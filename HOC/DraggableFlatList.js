...
class DraggableFlatList extends Component {
  ...
  constructor(props) {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: Animated.event(
        [null, { [props.horizontal ? 'moveX' : 'moveY']: this._moveAnim }],
        {
      ...
      onPanResponderRelease: () => {
        ...
        const position = horizontal
          ? pageX
          : pageY - this._scrollOffset + scrollOffset;
  ...
  animate = () => {
    ...
    const shouldScrollDown =
      this._move > this._containerSize * (1 - scrollRatio);
    ...
    if (
      ...
      if (positionPrevious >= 0) {
        ...
        if (move + this._scrollOffset < positionPrevious + sizePrevious / 2) {
          ...
          console.log(this._previousIndex, this._nextIndex);
        ...
        onLayout={e => {
          if (index !== this._tappedRow) {
            this._size[index] =
              ...
              ((firstItem && index === 0) || (!firstItem && isSpacerRow)
                ? this._tappedRowSize
                : 0);
            this.initPositions();
          }
        ...
        <Animated.View
          style={[
              ...
              : styles.hoverComponentVertical,
            {
              transform: [
                horizontal
                  ? { translateX: this._hoverAnim }
                  : { translateY: this._hoverAnim },
...
class RowItem extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <View
        collapsable={false}
        style={{ opacity: 1, flexDirection: horizontal ? 'row' : 'column' }}
      >
